...
void aes_hw_benchmark(void) {
    ...
    for (size_t size = 128; size <= 1024; size += 128) {
        ...
        uint64_t avg_inst   = total_instret / 10;
        uint64_t cycles_per_byte_x1000 = (avg_cycles * 1000) / size;

        printf("%zu, %llu, %llu, %llu.%03llu\n",
               size,
               avg_cycles,
               avg_inst,
